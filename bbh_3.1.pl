...
#OUTPUT
#BBH_of_query_N.txt. text file with orthologs for each query
use locale;
#use strict;
...
#		print@first_blast;
			...
			for ($blastline=0;  $blastline<@first_blast; $blastline++)  {
				print $blastline;
				if (@first_blast[$blastline] =~ /^Searching/)       {
#				print $line1;
				$line1 = @first_blast[$blastline+7];
				print "***$line1***\n";
				my $evalue= substr($line1, 76, 10);
				print"--$evalue\n\n";		
				chomp $evalue;
					if ($evalue<=1000 || $evalue ne "") {
					$line1 =~/^(.+?)\s/;
					$gi = $1;
					$line1 = $gi;                
					chomp($line1);                         			
	...
	#print @reverse_blast;
		...
		for ($j=0; $j<@reverse_blast; $j++){
			
			if (@reverse_blast[$j] =~/lcl\|(.+?)\s/){
			$putative_orth= $1;
			chomp($putative_orth);
			print "\n $putative_orth is a putative ortholog\n";
			}
			if (@reverse_blast[$j] =~ /^Sequences/){
			$line2 = @reverse_blast[$j+2];
			chomp($line2);
			$evalue2= substr($line2, 76, 10);
			print"evalue 2 $evalue2 \s\s";		
			chomp $evalue2;
			$line2 =~/^(.+?)\s/;
			
			$gi2 = $1;
			print "\n$gi_query - $gi2 - $evalue2\n";
			chomp ($gi2);
